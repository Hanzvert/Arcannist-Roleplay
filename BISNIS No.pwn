...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		{
			format(string, sizeof(string), "{00ffff}[ID: %d]\n{ffff00}%s\n"WHITE_E"Owner:{ffff00} %s", id, bData[id][bName], bData[id][bOwner]);
			bData[id][bPickup] = CreateDynamicPickup(19133, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 8.0);
			CreateDynamicCP(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 1.0, -1, -1, -1, 20.0);
        }
        else
        {
            format(string, sizeof(string), "[ID: %d]\n{00FF00}This bisnis for sell\n{FFFFFF}Location: {FFFF00}%s\n{FFFFFF}Price: {FFFF00}%s\n{FFFFFF}Type: {FFFF00}%s\n"WHITE_E"Type /buybisnis to purchase", id, GetLocation(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]), FormatMoney(bData[id][bPrice]), type);
            bData[id][bPickup] = CreateDynamicPickup(19133, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 8.0);
			CreateDynamicCP(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 1.0, -1, -1, -1, 20.0);
        }
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 2466.66, 2278.51, 92.13)) return Error(playerid, "Anda harus berada di City Hall!");
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "Menu Bisnis", CMDSString, "Select", "Cancel");
	return 1;
}
